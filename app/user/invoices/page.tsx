  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Invoices {data.length > 0 && `(${data.length})`}</h1>
        <Link href="/user/invoices/new" className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-colors shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50 border-b border-border uppercase">
                ...
                <th className="py-4 px-6 font-semibold text-xs tracking-wider text-muted-foreground">AMOUNT</th>
                <th className="py-4 px-6 font-semibold text-xs tracking-wider text-muted-foreground">STATUS</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-8 text-muted-foreground">No documents found. Create one!</td></tr>
              ) : currentData.map((item) => (
                <tr key={item.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                  <td className="py-4 px-6 text-sm font-medium text-primary">
                    <Link href={`/user/invoices/${item.id}/view`} className="hover:underline">
                      {item.docNumber}
                    </Link>
                  </td>
                  ...
                  <td className="py-4 px-6 text-sm font-medium text-foreground">
                    ₹{(item.totalAmount || item.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td className="py-4 px-6 text-sm">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${item.status === 'Paid' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                      {item.status}
                    </span>
                  </td>
                  <td className="py-4 px-6 text-center">
                    <Link href={`/user/invoices/${item.id}/view`} className="inline-flex items-center justify-center px-3 py-1.5 bg-[#e0e7ff] text-[#4338ca] hover:bg-[#c7d2fe] transition-colors rounded-md text-sm font-semibold whitespace-nowrap" title="View PDF">
                      <Eye className="w-4 h-4 mr-1.5" /> View
                    </Link>
                  </td>
                  <td className="py-4 px-6 flex items-center justify-end gap-4">
                    <Link href={`/user/invoices/${item.id}`} className="text-muted-foreground hover:text-foreground transition-colors" title="Edit">
