...
import { getLoadingSlipById, updateLoadingSlip } from "@/app/actions/documents";
...
export default function EditLoadingSlipPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    getLoadingSlipById(id).then(data => {
      if (data) {
        ...
        reset({
          ...
          vehicleNo: data.vehicleNo || d.vehicleNo || "",
          vehicleSize: d.vehicleSize || "",
          fromCity: d.fromCity || "",
          toCity: d.toCity || "",
          
          driverName: data.driverName || d.driverName || "",
          ...
          items: d.items && d.items.length > 0 ? d.items : [{ name: "", quantity: 1, condition: "Good", remark: "" }],
          
          detentionCharge: d.detentionCharge || 0,
          detentionType: d.detentionType || "",
          detentionRemark: d.detentionRemark || "",
          notes: d.notes || ""
        });
      }
  ...
  const onSave = async (data: any, actionType: 'submit' | 'send' = 'submit') => {
    setSaving(true);
    try {
      const result = await updateLoadingSlip(id, {
      ...
      if (actionType === 'send') {
        ...
        if (targetEmail) {
          ...
          showAlert("success", "Loading slip updated and emailed successfully!");
        } else {
      ...
      showAlert("error", err.message || "Failed to update loading slip");
    }
    setSaving(false);
  };
...
<div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Edit Loading Slip</h1>
        <button 
          type="button"
