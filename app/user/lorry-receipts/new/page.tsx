...
import { createLorryReceipt } from "@/app/actions/documents";
...
export default function NewLorryReceiptPage() {
  ...
  const onSave = async (data: any, actionType: 'submit' | 'send' = 'submit') => {
    setSaving(true);
    try {
      const result = await createLorryReceipt({
      ...
      if (actionType === 'send') {
        ...
        if (targetEmail) {
          ...
          showAlert("success", "Lorry Receipt created and emailed successfully!");
        } else {
      ...
      showAlert("error", err.message || "Failed to create lorry receipt");
    }
    setSaving(false);
  };
...
<div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Create LR / Bilty</h1>
        <button 
          type="button"
