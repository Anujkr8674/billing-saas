...
import { getMoneyReceipts, deleteMoneyReceipt } from "@/app/actions/documents";
...
export default function MoneyReceiptsPage() {
  ...
  const fetchData = async () => {
    const list = await getMoneyReceipts();
    setData(list);
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Money Receipts</h1>
          <p className="text-muted-foreground text-xs sm:text-sm truncate hidden sm:block">Manage your money receipts</p>
        </div>
        <Link 
          href="/user/money-receipts/new" 
        ...
        >
          <Plus className="w-5 h-5" /> Create Money Receipt
        </Link>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center flex flex-col items-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-1">No Money Receipts found</h3>
            <p className="text-muted-foreground mb-6">Create your first money receipt to get started.</p>
            <Link 
              href="/user/money-receipts/new" 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-muted/50 border-b border-border">
                    ...
                    <th className="py-4 px-6 font-semibold text-xs text-muted-foreground uppercase tracking-wider">Received From</th>
                  ...
                  {currentData.map((item) => (
                    <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                      <td className="py-4 px-6">
                        <Link href={`/user/money-receipts/${item.id}/view`} className="font-semibold text-[#5b21b6] hover:underline">
                          {item.docNumber}
                        </Link>
                      </td>
                      ...
                      <td className="py-4 px-6 text-sm font-medium text-foreground">{item.receivedFrom}</td>
                      ...
                      <td className="py-4 px-6 text-center">
                        <Link 
                          href={`/user/money-receipts/${item.id}/view`}
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center justify-end gap-2">
                          <Link 
                            href={`/user/money-receipts/${item.id}`}
