...
import { createNOCForm } from "@/app/actions/documents";
import { getUserProfile } from "@/app/actions/user";
import { toast } from "sonner";
import { sendNOCEmail } from "@/app/actions/email"; // To be created

export default function NewNOCFormPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: {
      date: new Date().toISOString().split("T")[0], // nocDate
      lrNo: "",
      lrDate: "",
      clientName: "",
      phone: "",
  ...
  const onSubmit = async (data: any) => {
    setLoading(true);
    try {
      // Store relevant data in details JSON to keep schema generic
      const payload = {
      ...
      const result = await createNOCForm(payload);
      toast.success("NOC Form created successfully!");
      ...
      toast.error(error.message || "Failed to create NOC Form");
    } finally {
      setLoading(false);
    }
  };
...
<div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Create NOC Form</h1>
        <button 
          type="button"
      ...
      <div className="bg-card p-4 border border-border rounded-xl shadow-sm flex flex-row overflow-x-auto justify-between items-center mt-6 gap-4">
        ...
        <div className="flex flex-row justify-end gap-2 sm:gap-3 shrink-0">
          ...
          >
            ...
            {loading && actionType === "save" ? "Saving..." : "Save Document"}
          </button>
          <button 
            type="button"
            onClick={() => {
