...
import { getNOCForms, deleteNOCForm } from "@/app/actions/documents";

const ITEMS_PER_PAGE = 10;

export default function NOCFormsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">NOC Forms</h1>
          <p className="text-muted-foreground text-xs sm:text-sm truncate hidden sm:block">Manage your No Objection Certificates</p>
        </div>
        <Link 
          href="/user/noc-forms/new" 
        ...
        >
          <Plus className="w-5 h-5" /> Create NOC
        </Link>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center flex flex-col items-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-1">No NOC Forms found</h3>
            <p className="text-muted-foreground mb-6">Create your first NOC to get started.</p>
            <Link 
              href="/user/noc-forms/new" 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-muted/50 border-b border-border">
                    ...
                    <th className="py-4 px-6 font-semibold text-xs text-muted-foreground uppercase tracking-wider">Client Name</th>
                  ...
                  {currentData.map((item) => (
                    <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                      <td className="py-4 px-6">
                        <Link href={`/user/noc-forms/${item.id}/view`} className="font-semibold text-[#5b21b6] hover:underline">
                          {item.docNumber}
                        </Link>
                      </td>
                      ...
                      <td className="py-4 px-6 text-sm font-medium text-foreground">{item.clientName}</td>
                      ...
                      <td className="py-4 px-6 text-center">
                        <Link 
                          href={`/user/noc-forms/${item.id}/view`}
