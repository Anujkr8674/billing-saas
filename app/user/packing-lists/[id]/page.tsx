...
import { getPackingListById, updatePackingList } from "@/app/actions/documents";
...
export default function EditPackingListPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    getPackingListById(id).then(data => {
      if (data) {
        ...
        reset({
          ...
          packingDate: data.date ? new Date(data.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
          shiftingDate: d.shiftingDate || "",
          fromCity: d.fromCity || "",
          toCity: d.toCity || "",
          vehicleNo: d.vehicleNo || "",
          packedBy: d.packedBy || "",

          items: d.items && d.items.length > 0 ? d.items : [{ name: "", boxNumber: "", quantity: 1, value: 0, cft: 0, remark: "" }],
          notes: d.notes || ""
        });
      }
  ...
  const onSave = async (data: any, actionType: 'submit' | 'send' = 'submit') => {
    setSaving(true);
    try {
      const result = await updatePackingList(id, {
      ...
      if (actionType === 'send') {
        ...
        if (targetEmail) {
          ...
          showAlert("success", "Packing List updated and emailed successfully!");
        } else {
      ...
      showAlert("error", err.message || "Failed to update packing list");
    }
    setSaving(false);
  };
...
<div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Edit Packing List</h1>
        <button 
          type="button"
