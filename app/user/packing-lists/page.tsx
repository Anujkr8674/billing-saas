...
import { getPackingLists, deletePackingList } from "@/app/actions/documents";
...
export default function PackingListsPage() {
  ...
  const fetchData = async () => {
    const list = await getPackingLists();
    setData(list);
    setLoading(false);
  };
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center flex flex-col items-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-1">No Packing Lists found</h3>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-muted/50 border-b border-border">
                    ...
                    <th className="py-4 px-6 font-semibold text-xs text-muted-foreground uppercase tracking-wider">Customer & Route</th>
                    <th className="py-4 px-6 font-semibold text-xs text-muted-foreground uppercase tracking-wider text-right">Items / Qty</th>
                  ...
                  {currentData.map((item) => (
                    <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                      <td className="py-4 px-6">
                        <Link href={`/user/packing-lists/${item.id}/view`} className="font-semibold text-[#5b21b6] hover:underline">
                          {item.docNumber}
                        </Link>
                      </td>
                      ...
                      <td className="py-4 px-6 text-sm">
                        <div className="font-bold text-foreground">{item.details?.customerName || "N/A"}</div>
                      ...
                      <td className="py-4 px-6 text-sm text-right font-medium">
                        {item.details?.items?.length || 0} Items <br />
                        <span className="text-xs text-muted-foreground">Qty: {item.details?.items?.reduce((acc: number, curr: any) => acc + (Number(curr.quantity) || 0), 0) || 0}</span>
                      </td>
                      ...
                      <td className="py-4 px-6 text-center">
                        <Link 
                          href={`/user/packing-lists/${item.id}/view`}
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center justify-end gap-2">
                          <Link 
                            href={`/user/packing-lists/${item.id}`}
