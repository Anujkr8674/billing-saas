...
import { createQuotation } from "@/app/actions/documents";
...
export default function NewQuotationPage() {
  ...
  const { register, control, handleSubmit, watch, setValue } = useForm({
    defaultValues: {
      ...
      items: [
        { name: "", quantity: 1, rate: 0, value: 0, remark: "" }
      ],
  ...
  const onSave = async (data: any, actionType: 'submit' | 'send') => {
    ...
    // Add calculated fields to details JSON before saving
    const finalData = {
      ...data,
      calculations: {
        freightTotal,
    ...
    try {
      const createdQuotation = await createQuotation({
      ...
      if (actionType === 'send') {
        await sendQuotationEmail(createdQuotation, userProfile, data.email);
        showAlert("success", "Quotation Saved & Sent via Email!");
      } else {
...
<div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Create Quotation</h1>
        <button 
          type="button"
