...
import { getQuotations, deleteQuotation } from "@/app/actions/documents";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Quotations {data.length > 0 && `(${data.length})`}</h1>
        <Link href="/user/quotations/new" className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-colors shadow-sm">
              ...
              {loading ? (
                ...
                <tr key={item.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                  <td className="py-4 px-6 text-sm font-medium text-primary">
                    <Link href={`/user/quotations/${item.id}/view`} className="hover:underline">
                      {item.docNumber}
                    </Link>
                  </td>
                  ...
                  <td className="py-4 px-6 text-sm">
                    ...
                    <div className="text-muted-foreground text-xs mt-0.5 flex items-center gap-1">
                      {item.details?.moveFrom?.city || "N/A"} <span className="text-muted-foreground">→</span> {item.details?.moveTo?.city || "N/A"}
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-6 text-center">
                    <Link href={`/user/quotations/${item.id}/view`} className="inline-flex items-center justify-center px-3 py-1.5 bg-[#e0e7ff] text-[#4338ca] hover:bg-[#c7d2fe] transition-colors rounded-md text-sm font-semibold whitespace-nowrap" title="View PDF">
                      <Eye className="w-4 h-4 mr-1.5" /> View
                    </Link>
                  </td>
                  <td className="py-4 px-6 flex items-center justify-end gap-4">
                    <Link href={`/user/quotations/${item.id}`} className="text-muted-foreground hover:text-foreground transition-colors" title="Edit">
