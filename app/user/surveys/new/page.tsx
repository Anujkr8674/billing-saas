...
import { createSurvey } from "@/app/actions/documents";
...
export default function NewSurveyPage() {
  ...
  const onSave = async (data: any, actionType: 'submit' | 'send' = 'submit') => {
    setLoading(true);
    try {
      // Map customerName to clientName for Prisma schema compatibility
      ...
      if (actionType === 'send') {
        ...
        await sendSurveyEmail(createdSurvey, userProfile, data.email);
      ...
      showAlert("error", err.message || "Failed to save survey");
    }
    setLoading(false);
  };
...
<div className="flex flex-row items-center justify-between gap-2 sm:gap-4 bg-card py-2 px-3 sm:px-4 rounded-lg shadow-sm border border-primary/20 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-bold text-foreground truncate">Create Survey</h1>
        <button 
          type="button"
      ...
      <form onSubmit={handleSubmit((data) => onSave(data, "submit"))} className="space-y-2">
        ...
        <div className="bg-card p-4 border border-border rounded-xl shadow-sm flex flex-row justify-between items-center mt-6 overflow-x-auto gap-2">
          ...
          <div className="flex flex-row gap-2 sm:gap-3 shrink-0">
            ...
            >
              {loading ? "Saving..." : "Submit & Send PDF"}
            </button>
          </div>
        </div>
