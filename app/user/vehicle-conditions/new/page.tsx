...
import { createVehicleCondition } from "@/app/actions/documents";
import { getUserProfile } from "@/app/actions/user";
import { toast } from "sonner";
import { sendVehicleConditionEmail } from "@/app/actions/email"; 

export default function NewVehicleConditionPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const result = await createVehicleCondition(payload);
      ...
      toast.error(error.message || "Failed to create Vehicle Condition Report");
    } finally {
      setLoading(false);
    }
  };
          ...
          {openSections.conditionDetails && (
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <input type="text" value="VC-Auto Generated" disabled className="w-full px-4 py-2.5 bg-muted/50 border border-border rounded-lg text-muted-foreground cursor-not-allowed" />
              </div>
              <div>
