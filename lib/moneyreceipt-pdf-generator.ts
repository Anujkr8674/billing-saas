...
export const generateMoneyReceiptPDF = async (receipt: any, userProfile: any): Promise<Blob> => {
  const doc = new jsPDF();
  ...
  doc.setFont("helvetica", "bold");
  doc.text("M O N E Y   R E C E I P T", pageWidth / 2, yPos, { align: "center" });
  ...
  doc.setTextColor(91, 33, 182);
  doc.setFont("helvetica", "bold");
  doc.text("RECEIPT DETAILS", 18, yPos + 6);
  doc.setDrawColor(240, 240, 240);
  doc.line(18, yPos + 8, 14 + boxWidth - 4, yPos + 8);
  
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.text(receipt.receivedFrom || "N/A", 18, yPos + 14);
  ...
  if (d.againstDocument) {
    doc.setTextColor(100, 100, 100);
    doc.text("Against Doc: ", 18, leftY);
    doc.setTextColor(0, 0, 0);
    doc.text(d.againstDocument, 18 + doc.getTextWidth("Against Doc: "), leftY);
  }
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Payment Information", box2X + 4, yPos + 5.5);
  
  const rightLines = [
    { label: "Receipt No:", val: receipt.docNumber },
    { label: "Date:", val: receipt.date ? new Date(receipt.date).toLocaleDateString("en-IN") : "N/A" },
    { label: "Payment Mode:", val: d.paymentMode || "Cash" },
    { label: "Amount:", val: `Rs. ${(receipt.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}` }
  ];
  ...
  rightLines.forEach(line => {
    ...
    doc.text(line.val, box2X + 30, rightY);
    rightY += 7;
  });
  ...
  if (d.amountInWords) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    const wordsLines = doc.splitTextToSize(`Words: ${d.amountInWords}`, boxWidth - 8);
    doc.text(wordsLines, box2X + 4, rightY);
  }
